<script lang="ts">
  ...
  import { loadSpotifyPlayer, STATUS, setDevice, getQueue } from './utils';
  //import SpotifyAuth from './internal/_auth/index.svelte';
  import { PKCE } from './_auth/pkce';
  import BasicPlayer from './BasicPlayer.svelte';
  ...
  $: {
    queue = queue;
    console.log(queue?.queue || 'queue not present!')
  };
  ...
  export async function login() {
    ...
    await authenticator.login().then((success) => {
      if (success) {
        isAuthorized = true;
  ...
  */
  export async function selectDevice() {
    ...
    queue = await getQueue(internalState.accessToken).then(response => response.json())
    console.log(queue)
    await setDevice(internalState.accessToken, internalState.deviceId);
  }
...
{#if !$$slots.all}
  ...
  {#if isAuthorized}
    ...
    {#if isReady && internalState.isActive}
      <slot name="player" queue={queue} player={player} state={playbackState}>
        <span>{queue?.queue?.length}</span>
        <BasicPlayer queue={queue} {player} state={playbackState} />
      </slot>
    {/if}
    {#if !isLoading}
